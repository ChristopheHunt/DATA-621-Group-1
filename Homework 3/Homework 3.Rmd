...
p_load(Hmisc, xlsx, xtable, knitr, scales, magrittr, tidyverse, 
       ...
       foreach, doParallel, stargazer, forecast, matrixStats, glmulti, leaps, data.table, highlight)
```
...
The purpose of this report is to develop a statistical model to determine the variables that are independently associated with neighborhoods with crime rates above or below the median. Note that neighborhoods with crime rates above or below the median have been provided in our evaluation data set.
...
```{r, fig.cap= "Correlation Plot of Training Data Set", echo = FALSE, fig.height = 4, fig.width= 6, cache=TRUE}
...
Using the `leaps` package and the `regsubsets` function we are able to subset our independent variables by looking at the best model for each predictor. Our final model using this method is line `8` which we will further implement into our subset selection model. 

```{r, subset, eval=TRUE, cache=TRUE, results = 'asis', echo=FALSE,  size="small"}
...
```{r, model from subset, eval=TRUE, cache=TRUE, echo=FALSE,size='small', results = 'asis'}
...
```{r, stepwise variable selection, eval=FALSE,  cache=TRUE, echo=FALSE}
library(glmulti)
library(stargazer)
glmultiLogisticOut <-
