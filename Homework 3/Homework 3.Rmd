...
fullModel <- lm(target ~ zn + I(log(indus)) + I(sqrt(chas)) + I(nox^-1) + I(log(rm)) + I(age^2) + 
                  I(dis^-.5) + rad + I(tax^-1) + I(ptratio^2) + I(black^2) + lstat + medv, 
                data = wCrimes) ##TODO - Review before submission
backFit <- glm(step(fullModel, direction = "backward", trace = F))
stargazer(backFit, header = FALSE)
```
...
forwardFit <- glm(formula = target ~ nox + rad + age + medv + ptratio, data = wCrimes, family = binomial)
stargazer(forwardFit, header = FALSE)
```
