...
Negative correlations for `nox`, `dis`, and `tax` are results of inverse transformations. Also, `black` coefficient appears as 0 in our table but is `-.000002870` which is too small of a number to be represented in the table. 
...
We will use a value of 5 as our threshold for multicollinearity of our variables[^8]. Here in our backwards selection model we find that ourt transformed `nox` and `dis` exceeds our pre-established threshold.
