...
[^6]: ["Research Crime Rates and the Impact on Home Values | Homes.com."](http://www.homes.com/real-estate/home-values/secure-new-home-research-crime-rates-impact-home-value/) Everything Real Estate Everything Homescom. N.p., 13 May 2016. Web. 30 Oct. 2016.

[^7]: Harrison, D. and Rubinfeld, D.L. 'Hedonic prices and the demand for clean air', J. Environ. Economics & Management, vol.5, 81-102, 1978.
...
We will use a value of 5 as our threshold for multicollinearity of our variables[^8]. Here in our backwards selection model we find that our transformed `nox` and `dis` exceeds our pre-established threshold.
...
```{r forward model metrics, echo=FALSE, results='asis', eval=TRUE}
...
```{r second forward model metrics, echo=FALSE, results='asis', eval=TRUE}
dfPred <- df[,c("nox","age","dis","rad","tax","ptratio", "black","lstat", "medv")]
predProb <- predict(fit, dfPred, type="response") #Predicted Probability
...
The variables as indicated in column `8` of the previous table will be further implement into our subset selection model in the following table. We don't see as strong of a relationship in this model as our previous model. For example, the coefficient for tax was as high as 106 in the forward selection model but is -5 in this model. However, our intercept in this model is larger than any other model.  
...
```{r subset model metrics, echo=FALSE, results='asis', eval=TRUE}
...
```{r subset model metrics second, echo=FALSE, results='asis', eval=TRUE}
dfPred <- df[,c("nox","age","dis", "rad","tax", "black", "lstat", "medv")]
predProb <- predict(fit, dfPred, type="response") #Predicted Probability
...
$$
\begin{aligned}
\log(target)~=~&-14.116*nox^{-1}~+~0.571*rad~+~0.227*age^{2}~-~0.0001*black^{2} \\
