...
```{r backwards model metrics, echo=FALSE, results='asis', eval=TRUE}
...
### Model Metrics for Forward Selection
...
Our results indicate that .610 would be the best threshold for this model so we re-run our metrics using this threshold. 
...
predResp <- numeric(length(predProb)) #Predicted Class
predResp[which(predProb>=0.610)] <- 1
...
```{r, subset, eval=TRUE, cache=TRUE, results = 'asis', echo=TRUE}
...
dfPred <- df[,c("zn", "nox","age","rad","tax","ptratio", "black", "medv")]
predProb <- predict(fit, dfPred, type="response") #Predicted Probability

predResp <- numeric(length(predProb)) #Predicted Class
predResp[which(predProb>=0.50)] <- 1

dfPred <- data.frame(cbind(df[,c("zn", "nox","age","rad","tax","ptratio", "black", "medv", "target")],predProb,predResp))

dfFwdModel <- ModelMetrics(dfPred,"predResp","target",1,0,"subset", fit$aic)
