...
```{r, offline, include=FALSE, cache=TRUE}
...
```{r, offline data, results='asis', echo = FALSE, cache=TRUE}
crimes <- dfTr
```
...
```{r, echo=FALSE, results='asis'}
fullModel <- lm(target ~ zn + indus + chas + nox + rm + age + dis + rad + tax + ptratio + black + lstat + medv, data = na.omit(wCrimes))
fit <- glm(step(fullModel, direction = "backward", trace = F))
stargazer(fit, header = FALSE)
```

We will use a value of 5 as our threshold for multicollinearity of our variables[^7]. Here in our backwards selection model we find that `nox` exceeds our pre-established threshold.
...
[^7]: ["Variance Inflation Factor (VIF)."](http://www.how2stats.net/2011/09/variance-inflation-factor-vif.html)  How2stats:. N.p., n.d. Web. 27 Oct. 2016.
...
subsetModel <- lm(target ~ nox + age + rad + tax + ptratio + black + lstat + medv, data = wCrimes)
