...
After competing the imputation, we can implement a correlation matrix to better understand the correlation between variables in the data set. The below matrix is our results and interestingly, Number of Wins appears to be most correlated to Base Hits by batters (1B,2B,3B,HR). 

```{r, fig.cap= "Correlation Plot of Training Data Set with imputed values", echo = FALSE}
imputed_df_m <- as.matrix(imputed_data$ximp)
...
lmfit = lm( data = imputed_df, `Number of wins` ~ `Homeruns by batters (4B)` + `Caught stealing` + `Strikeouts by batters`)
summary(lmfit)
plot(lmfit)
````
