...
We choose the Box Cox transformation for the following variables to improve linearity in our model. The lambda for the Box Cox transformation of our response variable is `r l1` which indicates that we should square the response variable to improve linearity. Also, we discovered that it was not necessary to transform Base Hits by Batters because the significance levels of the variable before and after transformation were the same. 
...
Our residuals in each model indcated Heteroskadistity and showed uneven variation. Therefore, we chose the model with the highest Adjusted R-squared which was Model 1. 

# TODO - update chart and make nice report - Christophe

```{r}
model <- c(1,2,3)
adjRSqrd <- c()
FStat <- c()
PValForFStat <- c('Significant', 'Significant', 'Significant')
ResidualVsFittedConstantVariation <- c('Not good,', 'Not good', 'Not good')
ResidualVsFittedCuve <- c('shows curve,', 'shows curve', 'shows curve')
ResidualVsFittedCuveHeteroscedasticity  <- c('yes,', 'yes', 'yes')
...
# TODO - explain inference - Senthil
...
# TODO - finish prediction section - Christophe

```{r, eval = FALSE}
eval_data <- function(x) as.numeric(as.character(x))
