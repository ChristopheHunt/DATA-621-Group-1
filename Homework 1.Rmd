...
Additionally, the skewness of each variable has been indicated in Table 1 - Descriptive Statistics. Two variables with a significant amount of skew are the number of base hits by batters and the number of walks allowed. Correspondingly, these two variables had a skew of 1.57 and 6.74 respectively. 
...
lmfit <- lm(data = imputed_df, 
           `Number of wins` ~ `Base Hits by batters (1B,2B,3B,HR)` + sqrt(`Strikeouts by batters`))

stargazer(lmfit, header = FALSE,  
          covariate.labels = c("Base Hits by batters (1B,2B,3B,HR)", "$\\sqrt{\\textnormal{Strikeouts~by~batters}}$"),
