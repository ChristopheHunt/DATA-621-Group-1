...
library(pacman)
p_load(Hmisc, xlsx, xtable, knitr, scales, magrittr, magrittr, tidyverse, stringr, e1071, corrplot, knitcitations, bibtex, missForest, abc, foreach, doParallel, stargazer, forecast, matrixStats, glmulti, leaps, data.table, highlight, car, pracma, boot, pander, ggplot2)
```
...
If we modify our data frame to a matrix in our evaluation data set we can further plot a correlation matrix. Unfortunately, we can only see a few interesting correlations in the data. 
...
```{r correlation matrix, fig.cap= "Correlation Plot of Training Data Set", echo = FALSE, fig.height = 7, fig.width= 12, cache=FALSE, eval=TRUE}
...
```{r residual plots, echo=FALSE, results='asis', cache=TRUE, eval=FALSE}
library(car)
