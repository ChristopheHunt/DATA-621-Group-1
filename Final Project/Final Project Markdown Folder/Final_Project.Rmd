...
stargazer(fit2, header = FALSE, no.space = TRUE, style = "all2", font.size = "normalsize", single.row = TRUE, intercept.bottom = FALSE, title = "Forward Selection Linear Model 1 with Influencial Points")
```
...
By excluding the values `70`, `115`, and `229` from our evaluation data set we are able to fit a more appropriate model. Below is the values in question we chose to exclude. 

```{r, echo = FALSE, results='asis', warning = FALSE, message = FALSE, cache=TRUE, fig.height=3.5, fig.width=10, eval=TRUE}
pandoc.table(dfLiquorSales[c(70,115,229),] %>% 
        select(Category.Name, Store.Name, Pack, Bottles.Sold, Sale..Dollars., 
               Bottle.Volume..ml., State.Bottle.Cost, State.Bottle.Retail, Volume.Sold..Gallons.), split.table = Inf, use.hyphening = TRUE, split.cells = 15)
```
...
```{r model 1, echo = FALSE, cache=TRUE, results = 'asis', eval = TRUE}
