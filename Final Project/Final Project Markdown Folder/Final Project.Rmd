...
We see that the dependent variable of Bottles Sold has a non-normal distribution as illustrated in the Cullen and Frey graph @CullenFrey4 from the `fitdistrplus` package and the non normal distribution is futher highlighted in the diagnostic plots.   
...
```{r, echo = FALSE, results='asis', warning = FALSE, message = FALSE, cache=TRUE, fig.height=4, fig.width=10, eval=TRUE}
fit.norm <- fitdist(iowaDataReduced$Bottles.Sold, "norm")
plot(fit.norm)
```

The lambda value of the BoxCox transformation of the Bottles Sold is `r round(BoxCox.lambda(as.numeric(iowaDataReduced$Bottles.Sold)),0)`. We therefore apply the log transformation of the dependent variable of Bottles Sold and see a more normal distribution.  
...
```{r forward, echo = FALSE, cache=TRUE}
...
forward <- step(lm(log(Bottles.Sold)~1,data=dfLiquorSales),direction = "forward",
                 ...
                 + State.Bottle.Cost + State.Bottle.Retail + Volume.Sold..Gallons.,trace = TRUE)
...
plot(fit2)
#Plots look better than fit1's plot but some of the predicted values are very high
coefficients(fit2)

#State.Bottle.Cost=One unit of increase in State.Bottle.Cost will increase 
#the log of bottles sold by 1.06 units

#Category.Name
# CANADIAN WHISKIES, IRISH, SCOTCH are highly significant. 
# In comparison to BLENDED WHISKIES, Canadian Whiskies will increase the log of bottles sold by 0.75
# In comparison to BLENDED WHISKIES, IRISH Whiskies will decrease the log of  bottles sold by 0.76
# In comparison to BLENDED WHISKIES, SCOTCH Whiskies will decrease the log of bottles sold by 0.60
# In comparison to BLENDED WHISKIES, SINGLE BARREL BOURBON Whiskies will decrease the log of  bottles sold by 0.68
# In comparison to BLENDED WHISKIES, STRAIGHT BOURBON Whiskies will decrease the log of  bottles sold by 0.11
# In comparison to BLENDED WHISKIES, STRAIGHT RYE Whiskies will decrease thelog of  bottles sold by 0.54
# In comparison to BLENDED WHISKIES, TENNESSEE WHISKIES  will decrease the log of bottles sold by 0.30

#State.Bottle.Retail=One unit of increase in State.Bottle.Retail will decrease the log of bottles sold by 0.70 units
#Pack=One unit of increase in Pack will increase the log of bottles sold by 0.06  units 
#Volume.Sold..Gallons.=One unit of increase in Volume.Sold..Gallons. will increase the log of bottles sold by 0.013 units
#Bottle.Volume..ml=One unit of increase in Bottle.Volume..ml will increase the log of bottles sold by 0.0005 units





plot(fit2$model$`log(Bottles.Sold)`,fit2$fitted.values)
