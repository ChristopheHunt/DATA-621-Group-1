...
stargazer(fit, header = FALSE,  
          no.space = TRUE, style = "qje", font.size = "scriptsize")
```
...
```{r forward model metrics, echo=FALSE, results='asis', eval=FALSE, cache=TRUE}
dfPred <- df[,c()]
...
subsetModel <- regsubsets(TARGET_FLAG ~ KIDSDRIV + AGE + HOMEKIDS + YOJ + I(INCOME^(1/4)) + PARENT1 + I(sqrt(HOME_VAL)) + MSTATUS + SEX + EDUCATION + JOB + TRAVTIME + CAR_USE + BLUEBOOK + TIF + CAR_TYPE + RED_CAR + I(log1p(OLDCLAIM)) + CLM_FREQ + REVOKED + MVR_PTS + CAR_AGE + URBANICITY, data = wimputedDfTr, nbest = 1)
...
```{r, model from subset, eval=FALSE, cache=TRUE, echo=FALSE, results = 'asis'}
...
stargazer(fit, header = FALSE, no.space = TRUE, style = "qje", font.size = "scriptsize", single.row = TRUE)
```
...
```{r, echo=FALSE, results='asis',  cache=TRUE, eval=FALSE}
