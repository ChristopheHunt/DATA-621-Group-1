...
```{r correlation matrix, fig.cap= "Correlation Plot of Training Data Set", echo = FALSE, fig.height = 5.5, fig.width= 10, cache=FALSE, eval=TRUE}
...
```{r box cox linear, echo=FALSE, results = 'asis', cache=TRUE}
library(car)
...
```{r box cox plot, echo=FALSE,cache=TRUE, message = FALSE}
residualPlots(fit)
```
...
```{r backwards selection log, echo=FALSE, results='asis',  cache=TRUE}
...
[^7]: ["Which Variance Inflation Factor Should I Be Using: $GVIF$ or $text{GVIF}^{1/(2cdottext{df})}$?"](http://stats.stackexchange.com/a/96584) R. N.p., n.d. Web. 13 Nov. 2016.
...
dfFwdModel <- ModelMetrics.LogReg(df = dfPred, predResp = "predResp", resp = "TARGET_FLAG",
                                  p = 1, n = 0,
                                  modelName = "Model Metrics for Forwards Selection with Best Threshold", fit = fit)
...
```{r multi forwards selection, echo=FALSE, results='asis',  cache=TRUE}
...
```{r sub set variable selection, subset, eval=TRUE, cache=TRUE, results = 'asis', echo=FALSE}
...
```{r sub set model, model from subset, eval=TRUE, cache=TRUE, echo=FALSE, results = 'asis'}
...
```{r subset model metrics with best threshold, echo=FALSE, results='asis', eval=TRUE,  cache=TRUE, error=TRUE}
dfPred <- wimputedDfTr[,c("KIDSDRIV", "INCOME", "MSTATUS", "JOB", "CAR_USE", "REVOKED", "MVR_PTS", "URBANICITY")]
predProb <- predict(fit, dfPred, type="response") #Predicted Probability

predResp <- numeric(length(predProb)) #Predicted Class
predResp[which(predProb>=0.280)] <- 1

dfPred <- data.frame(cbind(wimputedDfTr[,c("KIDSDRIV", "INCOME", "MSTATUS", "JOB", "CAR_USE", "REVOKED", "MVR_PTS", "URBANICITY", "TARGET_FLAG")], predProb, predResp))

dfSubModel <- ModelMetrics.LogReg(df = dfPred, predResp = "predResp", resp = "TARGET_FLAG",
                                  p = 1, n = 0,
                                  modelName = "Model Metrics for Subset Selection with Best Threshold", fit = fit)
...
```{r multi subset selection, echo=FALSE, results='asis',  cache=TRUE, eval=TRUE}
...
```{r backwards selection, echo = FALSE, results = 'asis', cache=TRUE, eval=TRUE}
...
```{r backwards selection model metrics, echo = FALSE, results = 'asis', cache=TRUE, eval=TRUE}
...
```{r forwards selection, echo = FALSE, results = 'asis', cache=TRUE, eval=TRUE}
fullModel <- lm(TARGET_AMT~1,data=wimputedDfTr)

fwdFitstep <- step(fullModel,direction = "both",
      scope=~KIDSDRIV + AGE + HOMEKIDS + YOJ + I(INCOME^(1/4)) + PARENT1 
     + MSTATUS + SEX + EDUCATION + JOB + I(sqrt(TRAVTIME)) + CAR_USE +  TIF 
...
```{r log model metrics, echo = FALSE, results = 'asis', cache=TRUE, eval=TRUE}
pander::pander(cbind(dfFwdModel, dfSubModel, dfBackModel), split.cell = 80, split.table = Inf)
```
...
```{r linear model metrics, echo = FALSE, results = 'asis', cache=TRUE, eval=TRUE}
pander::pander(cbind(fwdMLRMetrics, bkMLRMetrics), split.cell = 80, split.table = Inf)
```
...
```{r, results='asis', echo=FALSE, eval = FALSE,  cache=TRUE}
dfTrEval <- read.csv("https://raw.githubusercontent.com/ChristopheHunt/DATA-621-Group-1/master/Homework%204/insurance-evaluation-data.csv")

dfTrEval$JOB <- as.character(dfTrEval$JOB)

dfTrEval$JOB[(dfTrEval$JOB == "")] <- NA
...
dfTrEval <- dfTrEval %>%
        mutate(INCOME = removeDollars(INCOME),
               ...
               MSTATUS = as.factor(gsub("z_", "", MSTATUS)),
               SEX = as.factor(gsub("z_", "", SEX)),
               EDUCATION = factor((gsub("<", "Less Than ", 
                                        gsub("z_", "", EDUCATION))), 
...
dfTrEval <- dfTrEval %>% dplyr::select(-TARGET_FLAG,-TARGET_AMT)

imputed_data <- dfTrEval %>% missForest(maxiter = 10, ntree = 100, replace = TRUE, parallelize = 'forests', verbose = TRUE) 

write.csv(imputed_data$ximp,"imputed_insurance_evaluation_data.csv", row.names = FALSE) #wrote imputed_data to csv file due to processing time taken by missForest
```
...
imputeddfTrEval <- read.csv("https://raw.githubusercontent.com/ChristopheHunt/DATA-621-Group-1/master/Homework%204/imputed_insurance_evaluation_data.csv")
eval.PredProb <- predict(subsetModel, imputeddfTrEval ,type = "response") #Predicted Probability
TARGET_FLAG <- numeric(nrow(imputeddfTrEval)) 
TARGET_FLAG[which(eval.PredProb>=0.28)] <- 1 
imputeddfTrEval <- data.frame(cbind(imputeddfTrEval,eval.PredProb,TARGET_FLAG))
imputeddfTrEval$TARGET_AMT <- predict(fwdFitMLR, imputeddfTrEval)
imputeddfTrEval$TARGET_AMT <- dollar(imputeddfTrEval$TARGET_AMT)
pander::pander(imputeddfTrEval, split.cell = 30, split.table = Inf, round = 2)
```
