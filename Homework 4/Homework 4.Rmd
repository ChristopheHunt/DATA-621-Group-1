...
In the backward step selection modelm  The resulting AIC was . 

#TODO UPDATE THIS Negative correlations for `nox`, `dis`, and `tax` are results of inverse transformations. For example, as tax increases the log likelihood of the median crime rate increases, however we have inversed this relationship in this model. Also, `black` coefficient appears as 0 in our table but is `-.000002870` which is too small of a number to be represented in the table. 

```{r backwards model selection, echo=FALSE, results='asis',  cache=TRUE, eval=FALSE}
fullModel <- lm(TARGET_FLAG ~ KIDSDRIV + AGE + HOMEKIDS + YOJ + I(INCOME^(1/4)) + PARENT1 + I(sqrt(HOME_VAL)) + MSTATUS + SEX + EDUCATION + JOB + TRAVTIME + CAR_USE + BLUEBOOK + TIF + CAR_TYPE + RED_CAR + I(log(OLDCLAIM)) + CLM_FREQ + REVOKED + MVR_PTS + CAR_AGE + URBANICITY, data = wimputedDfTr) ##TODO - Review before submission
