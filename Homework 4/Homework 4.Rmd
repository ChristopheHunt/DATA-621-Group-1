...
```{r winsorize dataframe,  echo=FALSE, results='asis',  cache=TRUE, eval=FALSE}
library(forecast)
...
winzImputedDfTr <- winsorizedDataset(imputedDfTr) ##TODO - fix this for factors

stargazer(winzImputedDfTr, header = FALSE)
```
...
boxcoxSubDfTr <- as.data.frame(imputedDfTr %>% 
                            dplyr::select(INCOME, HOME_VAL, OLDCLAIM) %>% 
                            map_if(is.integer, as.numeric))

dfBoxCox <- boxcoxLambdas(boxcoxSubDfTr)
...
All other Lambda values were truncated to the nearest tenth that match a common transformation as per the below table.
