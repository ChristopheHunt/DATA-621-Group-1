...
boxplot(newmoneyball[,2:ncol(newmoneyball)])
skewness(newmoneyball)[c(1,2,11,ncol(newmoneyball))]
```

```{r}
layout(matrix(c(1,2,3,4), 2, 2))
hist(newmoneyball$TARGET_WINS)
hist(newmoneyball$TEAM_BATTING_H)
hist(newmoneyball$TEAM_PITCHING_BB)
hist(newmoneyball$TEAM_FIELDING_DP)
```
...
```{r}
library(forecast)
l1 = BoxCox.lambda(newmoneyball$TEAM_BATTING_H)
#l2 = BoxCox.lambda(newmoneyball$TEAM_FIELDING_DP)

newmoneyball$TEAM_BATTING_H_T = BoxCox(newmoneyball$TEAM_BATTING_H, l1)
#newmoneyball$TEAM_FIELDING_DP_T = BoxCox(newmoneyball$TEAM_FIELDING_DP, l2)

hist(newmoneyball$TEAM_BATTING_H_T)
#hist(newmoneyball$TEAM_FIELDING_DP_T)

skewness(newmoneyball$TEAM_BATTING_H_T)
#skewness(newmoneyball$TEAM_FIELDING_DP_T)

fit2 = lm(TARGET_WINS ~ TEAM_BATTING_H_T + TEAM_FIELDING_DP + TEAM_PITCHING_BB, data = newmoneyball)
summary(fit2)
layout(matrix(c(1,2,3,4), 2, 2))
plot(fit2)
```
